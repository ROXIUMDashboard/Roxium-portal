-- ============================================================
-- CATCH-UP / RECONCILE — bring a drifted live DB fully up to date.
--
-- Symptoms this fixes: "Could not add client: no unique or exclusion constraint
-- matching the ON CONFLICT specification"; team-member removal not working
-- (missing get_practice_roster / remove_practice_member); completion emails not
-- sending (missing practice_member_emails); empty/again-missing KPI columns.
--
-- SAFE + IDEMPOTENT. Run once in Supabase -> SQL Editor. It:
--   1) adds any missing COLUMNS / UNIQUE CONSTRAINTS that `create table if not
--      exists` cannot add to already-existing tables (Part A below), then
--   2) runs the full current schema.sql, which is written idempotently
--      (create table if not exists / create or replace function / drop policy
--      if exists + create / grants) and so refreshes every RPC, RLS policy and
--      index to current without touching your data (Part B below).
--
-- It NEVER deletes rows. The only caveat: if a `create unique index` fails with
-- "duplicate key", you have duplicate rows for that key — remove the extras and
-- re-run. Nothing else is destructive.
-- ============================================================

-- ---------- Part A: column + constraint reconcile (what schema.sql can't ALTER) ----------

-- sheet_sources: one row per (practice, channel) — the add-client ON CONFLICT target
alter table if exists sheet_sources drop constraint if exists sheet_sources_practice_id_key;
create unique index if not exists sheet_sources_practice_source_uq on sheet_sources (practice_id, source);
alter table if exists sheet_sources add column if not exists label text;
alter table if exists sheet_sources add column if not exists tab_name text;
alter table if exists sheet_sources add column if not exists gid text;
alter table if exists sheet_sources add column if not exists is_active boolean not null default true;

-- kpi_monthly: one row per (practice, month, source) + all metric columns the app reads
create unique index if not exists kpi_monthly_practice_period_source_uq on kpi_monthly (practice_id, period, source);
alter table if exists kpi_monthly add column if not exists source text not null default 'marketing';
alter table if exists kpi_monthly add column if not exists spend numeric;
alter table if exists kpi_monthly add column if not exists reach numeric;
alter table if exists kpi_monthly add column if not exists impr numeric;
alter table if exists kpi_monthly add column if not exists clicks numeric;
alter table if exists kpi_monthly add column if not exists lpv numeric;
alter table if exists kpi_monthly add column if not exists page_likes numeric;
alter table if exists kpi_monthly add column if not exists page_engagement numeric;
alter table if exists kpi_monthly add column if not exists foll numeric;
alter table if exists kpi_monthly add column if not exists cons numeric;
alter table if exists kpi_monthly add column if not exists proc numeric;
alter table if exists kpi_monthly add column if not exists finalized boolean not null default false;

-- profiles / memberships / practices: columns used by access + ops + join flows
alter table if exists profiles add column if not exists ops_attention_state jsonb;
alter table if exists profiles add column if not exists practice_id uuid;
alter table if exists practices add column if not exists join_code text;
alter table if exists practices add column if not exists workbook_sheet_id text;

-- ---------- Part B: full idempotent schema (refreshes every RPC / RLS / index) ----------
